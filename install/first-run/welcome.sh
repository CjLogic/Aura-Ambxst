...
notify-send "    Welcome to Aura!" "Opening welcome guide..." -u normal -t 3000

# Launch welcome modal in a floating terminal
# Check which terminal is available
if command -v kitty &>/dev/null; then
    kitty --class="aura-welcome" -e "$HOME/.local/share/aura/bin/aura-welcome-modal" &
elif command -v foot &>/dev/null; then
    foot --app-id="aura-welcome" "$HOME/.local/share/aura/bin/aura-welcome-modal" &
elif command -v ghostty &>/dev/null; then
    ghostty --class="aura-welcome" "$HOME/.local/share/aura/bin/aura-welcome-modal" &
